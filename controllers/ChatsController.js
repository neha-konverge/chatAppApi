...
const messagesImage = require('../models/Images')
    ...
    try {
        ...
        if (sender_id) {
            ...
            if (chatExist.length > 0) {
                res.send({ status: 1, chatUsers: chatUsers, message: "Chat messages list returned" ,count : chatUsers.length})
            } else {
...
const sendImage = async (req, res) => {
    try {
        const { reciever_id, sender_id, chat_id, message, attachment } = req.body
        if (sender_id && reciever_id && chat_id && attachment) {
            try {
                let newMsg = {
                    attachment : attachment,
                }
                newMsg = await messagesImage.create(newMsg)
                chatmsg = await MessageModel.create({
                    reciever_id: reciever_id,
                    sender_id: sender_id,
                    attachment : newMsg._id,  
                    type : 'image',                  
                    chat_id: chat_id 
                })

                res.send({ status: 1, messageSent: chatmsg, message: "Image sent" })
            } catch (error) {
                res.send({ message: error.toString() })
            }
        } else {
